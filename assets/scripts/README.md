...
- [一、整体架构](#一整体架构)
- [二、环境准备](#二环境准备)
- [三、选型指南：我该拉哪一套？](#三选型指南我该拉哪一套)
- [四、一键拉取](#四一键拉取)
- [五、开发模式一：手动引用技能 + Copilot/Codex](#五开发模式一手动引用技能--copilotcodex)
- [六、开发模式二：全自动开发闭环](#六开发模式二全自动开发闭环)
- [七、开发模式三：白板驱动开发](#七开发模式三白板驱动开发)
- [八、开发模式四：蜂群协作（多 AI 并行）](#八开发模式四蜂群协作多-ai-并行)
- [九、各业务线开发指南](#九各业务线开发指南)
- [十、自动化能力速查表](#十自动化能力速查表)
- [十一、常见问题](#十一常见问题)
...
回答以下问题，**第一个命中就是你的 profile**：

```
你的项目要接交易所 API 做自动交易吗？
  ├─ 是，加密货币 → quant-crypto
  ├─ 是，A 股     → quant-astock
  ├─ 是，美股     → quant-us
  └─ 否，继续 ↓
...
你的项目有"租户"概念吗？（不同客户/公司共用一套系统）
  ├─ 是 → saas
  └─ 否，继续 ↓

你的项目是企业内部系统吗？（ERP/OA/CRM/审批流）
  ├─ 是 → enterprise
  └─ 否 → full-stack（不确定就拉全量）
```
...
| 你的项目关键词 | Profile |
|---------------|---------|
| SaaS、多租户、计费、Stripe、订阅 | `saas` |
| ERP、OA、CRM、审批流、权限管理 | `enterprise` |
| 币安、OKX、加密货币、做市、套利 | `quant-crypto` |
| A 股、tushare、因子选股、回测 | `quant-astock` |
| 美股、Alpaca、Polygon、美股量化 | `quant-us` |
| Flutter、React Native、小程序、APP | `app-mini` |
| 不确定 / 多种混合 | `full-stack`
...
| 你是谁 | 推荐 | 理由 |
|--------|------|------|
| 全栈独立开发者 | `full-stack` | 技能越多越灵活 |
| 后端工程师（做 SaaS） | `saas` | 精准覆盖多租户/计费/认证 |
| 量化交易员 | `quant-crypto` / `quant-astock` | 数据源 + 策略 + 回测 + 风控 |
| 前端/移动端工程师 | `app-mini` | 跨平台开发全覆盖 |
| 企业 IT 负责人 | `enterprise` | 权限 + 工作流 + 微服务 |

### 不知道选什么？

```powershell
# 直接拉全量，多一点不影响
powershell -ExecutionPolicy Bypass -File D:\workspace\vibe-coding-cn\assets\scripts\bootstrap.ps1 -All
```

技能文件都是轻量 Markdown，全量也只有几十 MB，不会影响项目。
